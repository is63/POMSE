...
const preview = ref(user.value.foto ? (user.value.foto.startsWith('http') ? user.value.foto : 'https://localhost:8443/' + user.value.foto) : '/icons/favicon.svg')
const errorMsg = ref('')
...
onMounted(async () => {
    try {
        ...
        if (res.data) {
            ...
            preview.value = user.value.foto ? (user.value.foto.startsWith('http') ? user.value.foto : 'https://localhost:8443/' + user.value.foto) : '/icons/favicon.svg'
        }
    } catch { }
})
</script>
